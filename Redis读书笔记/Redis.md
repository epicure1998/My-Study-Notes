...
[==狂神说Redis==](https://www.bilibili.com/video/BV1S54y1R7SB?p=36)
...
### 哨兵模式(自动选举主机的模式) Sentinel

![sentinal](sentinel.png)

> `slaveof no one` 如果主机断开了，可以用这行命令手动变成主节点。如果此时原本主节点回来了，还得重新配置
>
> 费时费力，可以使用哨兵模式，监控后台主机是否故障，如果故障更具投票数==自动将从库变为主库==
...
> ==缓存穿透==：访问时，当用户搜索数据，服务器会先从缓存中查找数据是否存在，如果不存在则会去存储层的数据库中查找数据。那么如果访问网站的并发量非常大且都在第一层缓存中找不到数据，要访问存储层时，大量的并发会导致服务器支撑不住，从而造成宕机。在为服务中，服务的连接多为串联，一层服务崩了会导致一连串的==连锁效应==，导致整个服务器崩塌。这就是==缓存穿透==

##### 解决方案：

* 布隆过滤器：对所有可查询的参数以hash的形式存储，在控制层先进行校验，如果不存在或不符合则拒绝请求，从而避免对存储层的访问与压力

   * 有误差

* 缓存空对象：放存储层不命中时，即使返回了空对象也将其缓存起来，然后设置一个过期时间，之后别的客户端再访问这条数据时，直接在缓存中得到空值返回，从而保护了存储层

   * 控制被缓存意味着需要消耗更多的空间
   * 即使设置了过期时间，如果在过期时间内存储层存储了该数据，但用户还是查询不到，这样就造成了数据的不一致

#### 缓存击穿

> ==缓存击穿==：一个高热点的key被大量的并发访问，如果这个key被设置了失效时间，在失效时和再次加入缓存中有一个时间差，在这个时间差如果有巨大的并发访问，会全部穿过缓存直接访问存储层，容易使存储层压力过大导致宕机
>
> * 微博热搜，热点关键词访问了过大，宕机
> * 哪个明星又出轨了
...
*  设置热点数据不过期
   * 一直缓存会造成空间浪费的问题
* 加互斥锁，保证这个key在存储层中同一时间只能允许一个线程访问

#### 缓存雪崩

>==缓存雪崩==：在某一时间段，缓存集体过期失效，redis服务宕机。比如在双十二时，商品的高频访问时间是0点到1点这一个小时，所以将大部分商品数据的缓存过期时间设置为1小时，那么在1点时，大量商品的缓存数据集体过期，此时如果又有大量的并发访问，那么会直接访问数据存储层，导致服务器的压力过大
>
>==与缓存穿透的的区别==：缓存的穿透指的是一个key被大量的访问，而雪崩是大量的ke y失效后被再次高并发访问，单点和多点的区别，服务器都在承担大量的访问。所以我们需要服务器的高可用

##### 解决方案：

*  将部分服务关闭从而保证服务器的==高可用==，例如在双十一那天，淘宝的退货或其他的一些服务会被停掉，来减轻服务器的压力
* 多增设几台redis集群
* ==数据预热==：在正式部署之前，将可能成为热点的数据预先缓存一遍，这就是为什么有这么多的双十一预热活动，预热活动都可做热点数据的侦查。
* ==限流降级==
...
3. AOF常用配置总结
   ...
   >
   >* aof-load-truncated yes：如果AOF文件结尾损
   >
   >   坏，Redis启动时是否仍载入AOF文件

4. RDB和AOF的优缺点

5. redis常见性能问题和解决方案

   > Master最好不进行任何持久化的操作，RDB快照或是AOF的日志文件
   >
   > 如果数据很重要，就开启SLAVE进行AOF备份，策略设置为一秒一次
...
7. Reids三种不同删除策略
   ...
   >* 定时删除：设置键的过期时间，过期后自动执行删除
   >* 惰性删除：放任键过期不管，每次从键空间获取键是判断是否过期，过期的话，就删除
   >* 定期删除：定期对redis数据库缓存进行检查，删除过期键

8. Redis Module 实现布隆过滤器

   1. RedisBloom模块

9. Redis 到底是怎么实现“附近的人

   > 通过Geospatial进行实现，获取两个客户端的经纬度然后通过计算直接返回距离
